...
public class DavProducerMoveExistingTest extends AbstractDavTest {
	...
	template.sendBodyAndHeader(getDavUrl()
		+ "&moveExisting=${file:parent}/renamed-${file:onlyname}",
	...
	assertEquals(
		...
		context.getTypeConverter().convertTo(String.class,
			new File(DAV_ROOT_DIR + "/move/hello.txt")));
	...
	template.sendBodyAndHeader(
		getDavUrl()
			+ "&moveExisting=${file:parent}/renamed-${file:onlyname}&eagerDeleteTargetFile=true",
		"Old file", Exchange.FILE_NAME, "renamed-hello.txt");
	...
	template.sendBodyAndHeader(
		getDavUrl()
			+ "&moveExisting=${file:parent}/renamed-${file:onlyname}&eagerDeleteTargetFile=false",
		"Hello World", Exchange.FILE_NAME, "hello.txt");
	try {
	    template.sendBodyAndHeader(
		    getDavUrl()

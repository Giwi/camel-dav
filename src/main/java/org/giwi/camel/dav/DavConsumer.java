...
public class DavConsumer extends RemoteFileConsumer<DavResource> {
     ...
     * 
     * @see
     * org.apache.camel.component.file.GenericFileConsumer#pollDirectory(java
     * .lang.String, java.util.List, int)
     */
    @Override
	...
	return doPollDirectory(FileUtil.stripTrailingSeparator(fileName), null,
		fileList, depth);
    }
    ...
    protected boolean doPollDirectory(String absolutePath, String dirName,
	    List<GenericFile<DavResource>> fileList, int depth) {
	if (log.isDebugEnabled()) {
	    log.debug("doPollDirectory from absolutePath: {}, dirName: {}",
	...
	for (DavResource file : files) {
		...
		if (file.isDirectory()) {
		    RemoteFile<DavResource> remote = asRemoteFile(absolutePath,
			    file);
	...
	answer.setAbsoluteFilePath(FileUtil.stripLeadingSeparator(file
		.getPath()));
	...
	String relativefileName = FileUtil.stripLeadingSeparator(file.getPath()
		.replaceFirst(
			((RemoteFileConfiguration) endpoint.getConfiguration())
				.getInitialDirectory(), ""));

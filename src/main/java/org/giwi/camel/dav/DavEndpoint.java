...
public class DavEndpoint<T extends DavResource> extends RemoteFileEndpoint<DavResource> {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.giwi.camel.dav.RemoteFileEndpoint#buildConsumer(org.apache.camel. Processor)
	 */
	@Override
	...
	public RemoteFileOperations<DavResource> createRemoteFileOperations() throws Exception {
		...
		if (client == null) {
			...
			if (!"".equals(((DavConfiguration) configuration).getUsername())) {
				client = SardineFactory.begin(((DavConfiguration) configuration).getUsername(), ((DavConfiguration) configuration).getPassword());
			} else {
				client = SardineFactory.begin();
				// client.enablePreemptiveAuthentication(((DavConfiguration) configuration).getHost());
			}
		}
